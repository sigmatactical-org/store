...
impl ListingsStore {
    ...
    pub async fn update(&self, id: &str, input: UpdateListing) -> Result<Listing, StoreError> {
        ...
        let mut listing = self.get(id).await?.ok_or(StoreError::NotFound("listing"))?;
    ...
    async fn validate_sku_id(
        ...
        if sku_id.trim().is_empty() {
            return Err(StoreError::InvalidInput("sku_id is required".to_string()));
        }
        ...
        if exists {
            return Err(StoreError::InvalidInput(
                "listing for this catalog sku already exists".to_string(),
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    async fn reject_duplicate_sku_id() {
        ...
        assert_eq!(
            err.to_string(),
            "listing for this catalog sku already exists"
        );
    }
}
